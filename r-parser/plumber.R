...
#* Extract actions for a specific player grouped by skill (Base64 JSON)
#* @param req The request object
#* @post /player-actions-by-skill
#* @serializer json
function(req, res) {
  message("--- Incoming Request (/player-actions-by-skill) [JSON] ---")
  ...
  tryCatch(
    {
      ...
      # Use existing helper to get all player actions first
      actions <- extract_player_actions(dvw_data, team, number)

      # Filter for specific skills
      target_skills <- c("Attack", "Block", "Serve", "Reception", "Dig", "Set")
      actions_filtered <- actions[actions$skill %in% target_skills, ]

      # Group by skill
      # split returns a named list of data frames
      actions_by_skill <- split(actions_filtered, actions_filtered$skill)

      return(actions_by_skill)
    },
    error = function(e) {
      message("Error handling request: ", e$message)
      res$status <- 500
